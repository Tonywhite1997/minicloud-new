...
import useRenameFolder from "../../../customHooks/useRenameFolder";

function RenameFolder({ onClose, oldFolderName, folderID }) {
  const [folderName, setFolderName] = useState(oldFolderName || "");

  const { renameFolder, isPending, isSuccess, error } = useRenameFolder();

  const handleSubmit = () => {
    if (!folderName.trim()) return;
    renameFolder({ newFolderName: folderName, folderID });
    setFolderName("");
  };
